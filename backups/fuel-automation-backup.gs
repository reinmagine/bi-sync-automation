...
function startProcess() {
  ...
  for (let i = 1; i < responsibleData.length; i += 1) {
    ...
    if (!responsibleLookup[key]) {
      responsibleLookup[key] = sanitizeOutput(row[responsibleIndexes.responsible]);
    }
  }
  ...
  for (let r = 0; r < rowCount; r += 1) {
    ...
    const responsible =
      statusKey && responsibleLookup[statusKey]
        ...
        : '';
    if (statusKey && shouldExpectResponsible(statusKey) && !responsibleLookup[statusKey]) {
  ...
  outputHeaders.forEach((header) => {
    ...
    try {
      ...
      Logger.log('Error writing header %s at column %s: %s', header, column, e && e.message);
    }
  });
  ...
  if (lookupIssues.length > 0 || responsibleIssues.length > 0) {
    ...
    if (lookupIssues.length > 0) {
      writeRows.push(['--- Destination PLA ID mismatches (row, original, normalized, status) ---']);
      writeRows = writeRows.concat(lookupIssues);
    }
    if (responsibleIssues.length > 0) {
      writeRows.push(['']);
      writeRows.push(['--- STATUS mismatches where RESPONSIBLE is expected (row, original, normalized) ---']);
      writeRows = writeRows.concat(responsibleIssues);
    }

    // Ensure all rows have the same number of columns by padding shorter rows
    const maxCols = writeRows.reduce((m, r) => Math.max(m, r.length || 0), 0) || 1;
...
function isValidAmceoSheet(sheet) {
  ...
  return required.every((header) => headerMap[normalizeHeader(header)] !== undefined);
}
...
function mergeLookupRecord(existing, incoming) {
  return {
    territory: choosePreferredValue(existing && existing.territory, incoming && incoming.territory),
    siteClass: choosePreferredValue(existing && existing.siteClass, incoming && incoming.siteClass),
    tagging: choosePreferredValue(existing && existing.tagging, incoming && incoming.tagging),
  };
}
...
function computeTicketStatus(statusValue) {
  ...
  return (normalized === 'CLOSED' || normalized === 'REJECTED' || normalized === 'CANCELLED') ? 'CLOSED' : 'OPEN';
}
...
function computeOwner(projectTaggingValue) {
  ...
  if (normalized === 'GLOBE SITES' || normalized === 'BUILD GLOBE-ACQUIRED SITES') {
    return 'GLOBE';
  }
  if (normalized === 'BTS' || normalized === 'COLOCATION' || normalized === 'SLB') {
    return 'TOWERCO';
  }
  return '';
}
